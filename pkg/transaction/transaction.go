...
func NewTransaction(from, to string, amount, fee int64, utxoSet map[string][]TXOutput, privateKey *ecdsa.PrivateKey) (*Transaction, error) {
	var inputs []TXInput
	var outputs []TXOutput

	// Total requerido: cantidad + tarifa
	totalRequired := amount + fee

	// Reunir UTXOs para cubrir el total requerido
	accumulated, validOutputs := FindSpendableOutputs(from, totalRequired, utxoSet)
	...
	// Crear salidas: agregar la transacción principal
	outputs = append(outputs, TXOutput{Value: amount, ScriptPubKey: to})

	// Si hay cambio después de restar la tarifa, regresarlo al remitente
	if accumulated > totalRequired {
		outputs = append(outputs, TXOutput{Value: accumulated - totalRequired, ScriptPubKey: from})
	}
...
func deserializeTransaction(data string) *blockchain.Transaction {
